(ns stratus-clearpath.main
  "Kotodama WASM entrypoint for the Stratus_clearpath clean-room actor (L4) — Clojure port.
  ...
  facts (`stratus_clearpath.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "stratus_clearpath")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"stratus_clearpath.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "stratus_clearpath-compat" :tier tier :entities entities} 200])
...
(defn start! [] :stratus_clearpath-compat/ready)
